...
def transform_quaternions(data, t_pose_q, transforms, joint_heirarchy):
    ...
    for imu, parent in joint_heirarchy.items():
        ...
        raw_quaternions[f"{imu}_ss"] = (relative_rotations[4422:64412])
...
def plot_joint_angles(joint_angles, GRF, captury_joint_angles):
    ...
    for i, joint in enumerate(joint_angles.keys()):
        ...
        axes[0,i].plot(captury_joint_angle[t_cap:,0], linewidth=1, alpha = 0.5, color="red")
        axes[0,i].set_title(joint + "_X (deg)")
        
        # Plot Y axis
        axes[1,i].plot(joint_angle["angle_y"][t_ss:], linewidth=1, color="green")
        axes[1,i].plot(-captury_joint_angle[t_cap:,1], linewidth=1, alpha = 0.5, color="green")
...
def read_captury_data():
    csv_path = "/home/cshah/workspaces/sensorsuit/Captury logs/05_20_2025/sub01_walk_around.csv"
    ...
    for joint, (i0,i1,i2,i3) in joint_idxs.items():
        ...
        mat = df.iloc[2:28002, [i0, i1, i2, i3]].to_numpy()  
    ...
    for child, parent in joint_heirarchy.items():
        ...
        joint_angles_upsampled[child] = high_rate_rots.as_euler('xyz', degrees=True)
...
def main():
    ...
    csv_path = "/home/cshah/workspaces/sensorsuit/logs/05_20_2025/05_20_2025_walk_around.csv"
    ...
    captury_joint_angles, captury_joint_quaternions = read_captury_data()
    

    # # Plot the joint angles
    # plot_joint_angles(joint_angles, insole_force_data, captury_joint_angles)
    
    # Write all the data into one file for ML training
    # Utility.write_dataframe_2_csv(joint_rel_raw_quaternions, captury_joint_quaternions, acc_gyro_df)
